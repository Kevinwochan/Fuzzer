...
for filename in sorted(os.listdir('files')):
    ...
    for i in range(3):
        try:
            cmd = f'./fuzzer.py files/{filename} files/{filename}.txt Yes > /dev/null'
            if args.debug:
                cmd = f'./fuzzer.py files/{filename} files/{filename}.txt Yes'
            start = time.time()
            p = subprocess.run(cmd, timeout=int(args.timeout), shell=True)
            tdiff = time.time() - start
            times.append(tdiff)
            print(f'attempt {i+1} time taken: {tdiff}s')
        except:
            print('FAILED: timed out')
            test_summary.append([WARNING + filename, 'FAILED: timed out'])
            continue
    print(f'average time taken: {sum(times)/len(times)}s')
